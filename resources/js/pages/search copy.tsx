    ...
    return (
        <PublicLayout head="SHAULA">
            <div className="container mx-auto px-4">
                <div className="items-top mt-4 mb-6 flex justify-center">
                    {/* <div>
                    ...
                    <div>
                        <h1 className="mb-4 text-2xl font-bold">Resultados da busca por "{q}"</h1>
                        {last_page > 1 && (
                                    ...
                                    <div className="mb-2 flex flex-row items-center space-x-2">
                                        {item.primary_image_path && (
                                            <img
                                                ...
                                                className="aspect-square h-32 w-32 rounded object-cover"
                                            />
                                        )}
                                        <div className="flex flex-col">
                                            <p className="text-gray-500">{item.type ? typeLabelSearch(item.type) : ''}</p>
                                            <a href={item.route || '#'} className="text-lg font-semibold text-blue-600 hover:underline">
                                                {item.title || item.name || '[Sem título]'}
                                            </a>
                                            {item.authors && <p className="text-sm text-gray-600">{item.authors.join(', ')}</p>}
                                            {item.content && <p className="mt-2 text-gray-800">{item.content.substring(0, 150)}...</p>}
                                        </div>
                                    </div>
                                </li>
                            ))}
