...
export default function Images({
    ...
    const [imageToSelect, setImageToSelect] = useState<number>(historyArticle.data.primary_image?.id || 0);
                    ...
                    <div className="">
                        <form onSubmit={submit} className="space-y-6 bg-inherit">
                            ...
                            <div className="flex flex-row gap-2">
                                {historyArticle.data.images.map((image, index) => (
                                                ...
                                                onClick={
                                                    () => {
                                                        if (!imagesToRemove.find(id => id === image.id)) {
                                                            ...
                                                            if (image.id === historyArticle.data.primary_image?.id && imageToSelect === 0) {
                                                                setImageToSelect(historyArticle.data.primary_image?.id || 0);
                                                            }
                                                        }
                                                    }
                                                }
